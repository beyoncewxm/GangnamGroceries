...
<?php
	...
	if(isset($_SESSION['userid'])) {
		//store this item in user's recent field.
		storeRecent($_SESSION['userid'], $_SESSION['inventoryid']);
?>
...
<div>-->
	...
	<?php 
		if(isset($_SESSION['no_cart'])) {
			echo "<FONT COLOR=\"CC0000\">you must be logged in to add items to cart.<br /><br /></FONT>";
			unset($_SESSION['no_cart']);
		}
	
		if(isset($_SESSION['added'])) {
			echo "<FONT COLOR=\"CC0000\">item added to cart.<br /><br /></FONT>";
	...
	<form method='POST' action='addcart.php'>
		...
		<input type='text' name='qty' value='1' cols='3'><br />
		<?php echo "<input type='hidden' name='item' value='". $_GET['id'] ."'/>"?>
...
<?php
	$query2 = "SELECT * FROM gang.comment INNER JOIN gang.user_info USING (userid) WHERE inventoryid = $1";
        $stmt2 = pg_prepare($conn, "show_comment", $query2);

        if ($stmt2)
        {
		echo "Comments of the product: </br>";
                $result2 = pg_execute($conn, "show_comment", array($id));
		while ($row2 = pg_fetch_assoc($result2))
                {
                	echo "User: ".strtoupper(substr($row2['fname'], 0, 1)). substr($row2['fname'], 1)." ".strtoupper(substr($row2['lname'], 0, 1)).".</br>";
                	echo "comment: ".$row2['text']."</br><br>";
		}
	}
?>
...
<div id='content-wrapper'>
 <br />
        <form method='GET' action='selected_item.php'>
                <textarea name='commentfield' rows='5' cols='50'>Enter comments here.</textarea><br />
...
<?php
       // echo $_SESSION['inventoryid'];
        ...
        if(empty($_SESSION['userid']))
                ...
                echo "You haven't logged in, your comments will be anonymous.</br>";
...
<!--I switched the random item code for recently view item code -Scott-->
<?php 
/*	//IF user logged in
	if(isset($_SESSION['userid'])) {
?>
